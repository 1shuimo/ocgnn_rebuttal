...
def normalize_adj(adj):
    ...
    d_inv_sqrt = np.power(np.maximum(rowsum, 1e-8), -0.5).flatten()  # 避免对零值求倒数
...
def load_mat(dataset, train_rate=0.3, val_rate=0.1):
    ...
    abnormal_label_idx = normal_label_idx[: int(len(normal_label_idx) * 0.15)]  # 0.05 for Amazon and 0.15 for other datasets
