...
def _dataset_path(dataset):
    return DATASET_DIR / f"{dataset}.mat"
...
def  load_mat_10(dataset, train_rate=0.3, val_rate=0.1):
    """Load .mat dataset."""
    ...
    normal_label_idx = all_normal_label_idx[: 10]
    print('Training normal number 10')
    idx_test = list(set(all_idx) - set(normal_label_idx))
...
def load_mat_100(dataset, train_rate=0.3, val_rate=0.1):
    """Load .mat dataset."""
    data = sio.loadmat(_dataset_path(dataset))
    label = data['Label'] if ('Label' in data) else data['gnd']
    attr = data['Attributes'] if ('Attributes' in data) else data['X']
    ...
    all_normal_label_idx = [i for i in all_idx if ano_labels[i] == 0]
    random.shuffle(all_normal_label_idx)
    normal_label_idx = all_normal_label_idx[: 100]
    print('Training normal number 100')
    idx_test = list(set(all_idx) - set(normal_label_idx))
...
def load_mat(dataset, train_rate=0.1, val_rate=0.1):
    ...
    # add_rate = 0.15 * len(real_abnormal_id)  #0.05 0.1  0.15
    ...
    random.shuffle(normal_label_idx)
    # abnormal_label_idx = normal_label_idx[: 300]
    abnormal_label_idx = normal_label_idx[: int(len(normal_label_idx) * 0.15)]
    # abnormal_label_idx = normal_label_idx[: int(len(normal_label_idx) * 0.15)]
    # abnormal_label_idx = abnormal_label_idx[: int(num_train)]
    return adj, feat, ano_labels, all_idx, idx_train, idx_val, idx_test, ano_labels, str_ano_labels, attr_ano_labels, normal_label_idx, abnormal_label_idx
