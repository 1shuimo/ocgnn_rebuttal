...
add_log_subdir_argument(parser, 'single_mlp_forth')
...
def distillation_loss(emb_t, emb_s, mlp):
    teacher_logits = mlp(emb_t)
    student_logits = mlp(emb_s)
...
output_file = get_log_file(args, f"{args.dataset}_log_single_mlp.txt")
...
with open(output_file, "a") as f:
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            teacher_loss, _, c, r= loss_func(emb_t_normal)
            ...
            distillation_loss_value = distillation_loss(emb_t_all, emb_s_all, mlp)
            ...
            if epoch % 5 == 0:
                ...
                student_score = mlp(emb_s).squeeze(dim=-1) # Use MLP to calculate student model score
                student_score = torch.sigmoid(student_score)
                
                teacher_score = mlp(emb_t).squeeze(dim=-1)
