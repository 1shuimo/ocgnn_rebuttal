...
add_log_subdir_argument(parser, 'emb_add_mlp_forth')
...
# Define distillation loss function
def distillation_loss_mlp(emb_t, emb_s, mlp_t, mlp_s):
...
output_file = get_log_file(args, f"{args.dataset}_log_emb_add_mlp.txt")
...
with open(output_file, "a") as f:
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            # Calculate distillation loss (add MLP)
            distillation_loss_mlp_value = distillation_loss_mlp(emb_t_all, emb_s_all, mlp_t, mlp_s)

            # Combine distillation losses
            distillation_loss_value = distillation_loss_emb_value + distillation_loss_mlp_value
            ...
            if epoch % 5 == 0:
                ...
                # Calculate melt score for each node
                emb_score = torch.mean((emb_s - emb_t) ** 2, dim=1)
                ...
                stu_add_emb_score = student_score + emb_score
                ...
                stu_emb_ocgnn_score = stu_add_emb_score + score_from_ocgnn
                
                
                # Extract scores for test indices
                logits_emb = np.squeeze(emb_score[idx_test].cpu().detach().numpy())
                ...
                logits_stu_emb = np.squeeze(stu_add_emb_score[idx_test].cpu().detach().numpy())
                ...
                logits_stu_emb_ocgnn = np.squeeze(stu_emb_ocgnn_score[idx_test].cpu().detach().numpy())


                # Calculate AUC for different logits
                auc_emb = roc_auc_score(ano_label[idx_test], logits_emb)
                ...
                auc_stu_emb = roc_auc_score(ano_label[idx_test], logits_stu_emb)
                ...
                auc_stu_emb_ocgnn = roc_auc_score(ano_label[idx_test], logits_stu_emb_ocgnn)

                log_message += f'Testing {args.dataset} AUC_emb: {auc_emb:.4f}\n'
                log_message += f'Testing {args.dataset} AUC_student_mlp: {auc_stu:.4f}\n'
                log_message += f'Testing {args.dataset} AUC_teacher_mlp: {auc_tea:.4f}\n'
                log_message += f'Testing {args.dataset} AUC_student_teacher_mlp: {auc_stu_tea:.4f}\n'
                log_message += f'Testing {args.dataset} AUC_student_emb_mlp: {auc_stu_emb:.4f}\n'
                ...
                log_message += f'Testing {args.dataset} AUC_student_emb_ocgnn: {auc_stu_emb_ocgnn:.4f}\n'

                # Calculate AP for different logits
                AP_emb = average_precision_score(ano_label[idx_test], logits_emb, average='macro', pos_label=1)
                ...
                AP_stu_emb = average_precision_score(ano_label[idx_test], logits_stu_emb, average='macro', pos_label=1)
                ...
                AP_stu_emb_ocgnn = average_precision_score(ano_label[idx_test], logits_stu_emb_ocgnn, average='macro', pos_label=1)

                log_message += f'Testing AP_emb: {AP_emb:.4f}\n'
                log_message += f'Testing AP_student_mlp: {AP_stu:.4f}\n'
                log_message += f'Testing AP_teacher_mlp: {AP_tea:.4f}\n'
                log_message += f'Testing AP_student_teacher_mlp: {AP_stu_tea:.4f}\n'
                log_message += f'Testing AP_student_emb_mlp: {AP_stu_emb:.4f}\n'
                ...
                log_message += f'Testing AP_student_emb_ocgnn: {AP_stu_emb_ocgnn:.4f}\n'
