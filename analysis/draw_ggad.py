...
def draw_score_distribution(scores_normal, scores_abnormal, model_type, dataset_name, file_suffix, auroc=None, auprc=None):
    ...
    # 保存为SVG格式
    save_path_svg = f'{save_dir}/{model_type}_{file_suffix}_0.92.svg'
    plt.savefig(save_path_svg, format='svg', bbox_inches='tight')
    
    # 保存为PDF格式
    save_path_pdf = f'{save_dir}/{model_type}_{file_suffix}_0.92.pdf'
...
with torch.no_grad():
    ...
    for mode, (student_scores, teacher_scores) in score_modes.items():
        ...
        if all(len(arr) > 0 for arr in [s_normal, s_abnormal]):
            draw_score_distribution(s_normal, s_abnormal, 'student', args.dataset, f"final_{mode}", auroc=0.961, auprc=0.840)
        else:
            print(f"❌ Skipping student ({mode}) plot - one or more node categories are empty.")

        if all(len(arr) > 0 for arr in [t_normal, t_abnormal]):
            draw_score_distribution(t_normal,t_abnormal, 'teacher', args.dataset, f"final_{mode}", auroc=0.961, auprc=0.840)
        else:
