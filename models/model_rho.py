...
class RHO(nn.Module):
    def __init__(self, in_features, hidden1, hid2, layers, batch_size, tau):
        ...
        self.encoder = MLPEncoder(in_features, hidden1, hid2)
    ...
    def forward(self, Lap, x):
        ...
        embedding_g_p = self.proj_head1(x_global)
        embedding_l_p = self.proj_head2(x_local)
    ...
    def batch_nce_loss(self, z1, z2, temperature=0.2, pos_mask=None, neg_mask=None):
        ...
        if (self.batch_size == 0) or (self.batch_size > nnodes):
            pos_mask = torch.eye(z1.shape[0], device=z1.device)
            neg_mask = 1 - pos_mask
    ...
    def infonce(self, anchor, sample, pos_mask, neg_mask, tau):
        ...
        log_prob = sim - torch.log(exp_sim.sum(dim=1, keepdim=True)) - torch.log(
            exp_sim_anchor.sum(dim=1, keepdim=True)
        )
