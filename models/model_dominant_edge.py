...
class Model_dominant(nn.Module):
    def __init__(self, n_in, n_h, activation, negsamp_round, readout, dataset_name= None):
        super(Model_dominant, self).__init__()
        self.read_mode = readout
        self.dense_stru = nn.Linear(n_in, n_h)
        self.gat_layer = GCN(n_h, n_h, num_layers=2)
    ...
    def forward(self, seq1, edge_index, idx_train, idx_test): # <- 变化1: 接收 edge_index 而不是 adj
            # adj = torch.squeeze(adj) # <- 变化2: 删除 adj 相关的所有处理
            seq1 = torch.squeeze(seq1)
            # edge_list = neighList_to_edgeList(adj) # <- 变化2: 删除这两行低效代码
            # edge_index = torch.tensor(np.array(edge_list)).T

            # model_enc 现在直接使用传入的 edge_index，无需任何改动
            x_, s_ = self.model_enc(seq1, edge_index)

            # 您的 loss 函数只计算了属性损失，所以我们可以直接传入 None 作为结构部分
            score = self.double_recon_loss(seq1[idx_train, :],
                                           x_[idx_train, :],
                                           None, # <- 变化3: 因为没有 adj, 传入 None
                                           None, # <- 变化3: 因为没有 s_, 传入 None
                                           )
            ...
            return loss, score, score_test, self.emb, seq1, x_