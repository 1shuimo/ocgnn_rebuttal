...
def get_all_normal_nodes(emb_s_all, teacher_hard):
    """
    获取所有normal节点的索引
    所有normal节点 = emb_s_all的所有节点 - top5%异常节点
    """
    total_nodes = emb_s_all.shape[0]  # 这是原始图的节点数
    all_idx = set(range(total_nodes))
    
    # 排除top5%异常节点
    top5_abnormal_idx = set(torch.where(teacher_hard == 1)[0].cpu().numpy())
    
    # 所有normal节点 = 全部节点 - top5%异常
...
output_file = f"./ggad_2_step_2_reg/{args.dataset}_reg2_bce.txt"
with open(output_file, "a") as f:
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            # 5. 正则项
            reg_loss = torch.norm(pseudo_emb_proj - hard1_emb_center, p=2, dim=1).mean()

            # 获取所有normal节点（emb_s_all中的节点 - top5%异常）
            all_normal_idx = get_all_normal_nodes(emb_s_all, teacher_hard)

            # === 计算reg2 - 对所有normal节点做数据增强 ===
            # 1. 获取所有normal节点的特征
            all_normal_features = features.squeeze()[all_normal_idx]  # [N_all_normal, D]

            # 2. 对这些特征进行random mask (30%概率置0)
            ...
            masked_features = masked_features.unsqueeze(0)  # 恢复batch维度

            # 4. 把masked特征过student network得到augmented embedding
            ...
            total_loss = bce_loss + 0.1* reg_loss + 0.01 * reg2_mse
            ...
            if epoch % 5 == 0:
                ...
                log_message += (
                    ...
                    f"BCE Loss = {bce_loss.item()}\n"
                    f"Reg2 Loss = {reg2_mse.item()}\n"
                )                
                model_s.eval()
