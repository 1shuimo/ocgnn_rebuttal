...
add_log_subdir_argument(parser, 'ggad_labeledNormal_rebuttal_full')
...
def format_distance_metrics(prefix, metrics):
    return (
        f"{prefix} real_abnormal_center -> labeled_normal_center: "
...
def format_ratio_metrics(labels, logits, base_ratio):
    lines = []
    ratios = [base_ratio - 0.1, base_ratio, base_ratio + 0.1]
    ...
    for ratio in ratios:
        ...
        lines.append(
            ...
            f"PredictedAbnormalCount={int(predicted_mask.sum())}"
        )
    return "\n".join(lines) + "\n"
...
with torch.no_grad():
    ...
    teacher_distance_metrics = compute_distance_metrics(
        emb_t_all, ano_label, normal_label_idx
    )
    log_message_initial += format_distance_metrics("Teacher", teacher_distance_metrics)

print(
    f"NormReg enabled: {bool(args.use_normreg)} "
    f"(weight={args.normreg_weight if args.use_normreg else 0.0})"
)
...
best_student_metrics_file = get_log_file(args, f"{args.dataset}_{run_tag}_best_metrics.txt")
best_auc = float("-inf")
with open(output_file, "a") as f:
    ...
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            # Step 1: (all_normal_idx 已在循环外计算好)
            ...
            student_distance_metrics = compute_distance_metrics(
                emb_s_all.detach(), ano_label, normal_label_idx
            )
            log_message += format_distance_metrics("Student", student_distance_metrics)
            log_message += format_ratio_metrics(ano_label[idx_test], logits_stu, args.base_ratio)
            ...
            if auc_stu > best_auc:
                ...
                np.save(best_labels_file, ano_label[idx_test])
                with open(best_student_metrics_file, "w") as best_f:
                    best_f.write(f"Best AUC_student_mlp_s: {best_auc:.4f}\n")
                    best_f.write(f"NormReg enabled: {bool(args.use_normreg)}\n")
                    best_f.write(f"Base ratio: {args.base_ratio:.2f}\n")
                    best_f.write(format_distance_metrics("Teacher", teacher_distance_metrics))
                    best_f.write(format_distance_metrics("Best Student", student_distance_metrics))
                    best_f.write(format_ratio_metrics(ano_label[idx_test], logits_stu, args.base_ratio))
