...
add_log_subdir_argument(parser, 'ocgnn_2_step_score_emb')
    ...
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            # auc of teacher model
                loss, teacher_score, _, _ = loss_func(emb_tea_2)
                teacher_score = min_max_normalize(teacher_score)
                # evaluation on the valid and test node
                logits = np.squeeze(teacher_score[idx_test].cpu().detach().numpy())
            ...
            student_score = mlp_s(emb_stu_2_norm).squeeze(dim=-1)
            student_score = torch.sigmoid(student_score)

            score_loss = score_distillation_loss(student_score, teacher_score)
            ...
            total_loss = (mse_loss_1 + mse_loss_2)+ score_loss 
            ...
            if epoch % 5 == 0:
                ...
                log_message += f"emb_loss = {(mse_loss_1 + mse_loss_2).item()}\n"
                model_s.eval()
                model.eval()
