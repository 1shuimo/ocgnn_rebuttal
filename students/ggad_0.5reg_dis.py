...
add_log_subdir_argument(parser, 'ggad_melt_abnormal')
...
output_file = get_log_file(args, f"{args.dataset}_melt_abnormal_optimize_{args.lr}_0.5reg.txt")
with open(output_file, "a") as f:
    ...
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            # === 总损失计算 ===
            total_loss = mse_loss  + 0.5 * reg2_mse
            #                ^^^^^^^^^^^^    ^^^^^^^^^^^^^^^^^^^^
            ...
            if epoch % 5 == 0:
                ...
                # ====================================================================================
                # ✨ 更新: 仅使用测试集内部的正常节点计算中心和距离
                # ====================================================================================
                with torch.no_grad():
                    # 1. 筛选出测试集中的真实正常节点
                    test_indices_np = np.array(idx_test)
                    test_labels_np = ano_label[test_indices_np]
                    test_normal_indices = test_indices_np[test_labels_np == 0]

                    sum_dist, avg_dist = 0.0, 0.0
                    # 确保测试集中有正常节点，以避免除以零的错误
                    if len(test_normal_indices) > 0:
                        # 2. 获取这些测试集正常节点的嵌入
                        test_normal_embeddings = emb_s_all[test_normal_indices]
                        
                        # 3. 计算“测试集正常中心”
                        test_normal_center = torch.mean(test_normal_embeddings, dim=0)

                        # 4. 计算每个测试集正常节点到该中心的欧几里得距离
                        distances = torch.sqrt(torch.sum((test_normal_embeddings - test_normal_center) ** 2, dim=1))
                        sum_dist = torch.sum(distances).item()
                        avg_dist = torch.mean(distances).item()

                log_message += f'Sum of test normal distances to TEST-SET center: {sum_dist:.4f}\n'
                log_message += f'Average test normal distance to TEST-SET center: {avg_dist:.4f}\n'
                # ====================================================================================
                ...
                if auc_stu >= 0.65:
                    print(f"\n🎉 AUC threshold of 0.84 reached! Stopping training at epoch {epoch}.")
                    break
...
student_model_path = os.path.join(save_dir, f"student_main{args.dataset}_lr{args.lr}_0.5_reg.pth")
mlp_path = os.path.join(save_dir, f"mlp_lr{args.lr}_0.5_reg.pth")
