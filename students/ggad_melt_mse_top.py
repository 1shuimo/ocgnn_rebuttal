...
def to_hard_label_exclude_normal(score, normal_idx, top_percent):
    """
    ...
    top_k = int(len(all_idx) * top_percent)  # 计算top%的节点数量
    if top_k < 1:
        top_k = 1
...
def get_all_normal_nodes(emb_s_all, teacher_hard):
    ...
    # 找出top%异常节点的索引
    top_abnormal_idx = set(torch.where(teacher_hard == 1)[0].cpu().numpy())
    
    # 所有normal节点 = 全部节点 - top%异常节点
    all_normal_idx = list(all_idx - top_abnormal_idx)
...
# 预处理特征
if args.dataset in ['Amazon', 'tf_finace', 'reddit', 'elliptic','question']:
...
with open(output_file, "a") as f:
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            # === 生成硬标签 ===
            # 将硬编码的 top_percent 替换为命令行参数
            teacher_hard = to_hard_label_exclude_normal(score_from_ggad_non_normalize, normal_label_idx, top_percent=args.top_percent)  # [N]
            hard1_indices = torch.where(teacher_hard == 1)[0]  # top%异常节点索引 [K]
            ...
            # === 总损失计算 ===
            # 将硬编码的 0.01 替换为命令行参数
            total_loss = mse_loss + args.reg2_weight * reg2_mse
            #            ^^^^^^^^    ^^^^^^^^^^^^^^
