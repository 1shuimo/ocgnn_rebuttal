...
add_log_subdir_argument(parser, 'student_only_distill_find_fp')
...
with torch.no_grad():
    ...
    # 4. 获取teacher embedding和伪异常embedding (用于学生模型输入)
...
output_dir = "./student_only_distill_find_fp"

# Create the directory if it doesn't exist
os.makedirs(output_dir, exist_ok=True)
# Setup for training and logging
output_file = get_log_file(args, f"{args.dataset}_student_only_distill_find_fp_fn.txt")
best_logits_file = f"./student_only_distill_find_fp/{args.dataset}_best_logits.npy"  # 保存最佳 logits 的文件路径
best_labels_file = f"./student_only_distill_find_fp/{args.dataset}_test_labels.npy"  # 保存测试集真实标签的文件路径
with open(output_file, "a") as f:
    ...
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            # 对伪异常embedding进行变换 (这部分需要在循环内，因为pseudo_emb_mlp在训练)
            ...
            if epoch % 5 == 0:
                ...
                # teacher的分数已在循环外计算好，直接使用
                # score_from_ggad, score_from_ggad_non_normalize
                ...
                logits_stu_ggad_non_normalize = np.squeeze(
                    stu_add_ggad_non_normalize_score[idx_test].cpu().detach().numpy())
                logits_stu_minus_ggad = np.squeeze(student_minus_ggad_score[idx_test].cpu().detach().numpy())
                logits_stu_minus_ggad_non_normalize = np.squeeze(
                    student_minus_ggad_non_normalize_score[idx_test].cpu().detach().numpy())
                ...
                # === 根据固定比例预测异常节点 ===
                # 1. 获取测试集的总节点数
                num_total = len(idx_test)  # 测试集总节点数

                # 2. 显式设置异常比例为 20%
                abnormal_ratio = 0.2  # 前 20% 的节点标记为异常

                # 3. 排序 logits_test
                sorted_indices = np.argsort(logits_stu)[::-1]  # 从大到小排序
                predicted_abnormal_count = int(abnormal_ratio * num_total)  # 预测的异常节点数量
                predicted_abnormal_indices = sorted_indices[:predicted_abnormal_count]  # 预测为异常的节点索引
                predicted_normal_indices = sorted_indices[predicted_abnormal_count:]  # 剩余的节点标记为正常

                # === 统计正常节点被错误分类为异常的数量 (False Positive) ===
                # 获取真实的正常节点索引
                true_normal_indices = np.where(ano_label[idx_test] == 0)[0]  # 真实正常节点索引（相对于测试集）

                # 预测为异常的节点中，真实正常节点的数量
                false_positive_count = len(set(predicted_abnormal_indices) & set(true_normal_indices))

                # 记录 False Positive 结果
                log_message += f"False Positive Count (Normal -> Abnormal): {false_positive_count}\n"
                log_message += f"Predicted Abnormal Count: {predicted_abnormal_count}\n"

                # === 统计异常节点被错误分类为正常的数量 (False Negative) ===
                # 获取真实的异常节点索引
                true_abnormal_indices = np.where(ano_label[idx_test] == 1)[0]  # 真实异常节点索引（相对于测试集）

                # 预测为正常的节点中，真实异常节点的数量
                false_negative_count = len(set(predicted_normal_indices) & set(true_abnormal_indices))

                # 记录 False Negative 结果
                log_message += f"False Negative Count (Abnormal -> Normal): {false_negative_count}\n"
                ...
                auc_stu_minus_ggad_non_normalize = roc_auc_score(ano_label[idx_test],
                                                                 logits_stu_minus_ggad_non_normalize)
                # 保存最佳 logits 和标签
                if auc_stu > 0.71:
                    best_auc = auc_stu
                    best_epoch = epoch
                    np.save(best_logits_file, logits_stu)  # 保存最佳 logits
                    np.save(best_labels_file, ano_label[idx_test])  # 保存测试集真实标签
                    print(f"✅ Best AUC updated: {best_auc:.4f} at epoch {best_epoch}")
                    print(f"Logits saved to: {best_logits_file}")
                    print(f"Labels saved to: {best_labels_file}")
                    break
                ...
                AP_stu_non_normalize = average_precision_score(ano_label[idx_test], logits_stu_non_normalize,
                                                               average='macro', pos_label=1)
                AP_stu_ggad = average_precision_score(ano_label[idx_test], logits_stu_ggad, average='macro',
                                                      pos_label=1)
                AP_stu_ggad_non_normalize = average_precision_score(ano_label[idx_test], logits_stu_ggad_non_normalize,
                                                                    average='macro', pos_label=1)
                AP_stu_minus_ggad = average_precision_score(ano_label[idx_test], logits_stu_minus_ggad, average='macro',
                                                            pos_label=1)
                AP_stu_minus_ggad_non_normalize = average_precision_score(ano_label[idx_test],
                                                                          logits_stu_minus_ggad_non_normalize,
                                                                          average='macro', pos_label=1)
