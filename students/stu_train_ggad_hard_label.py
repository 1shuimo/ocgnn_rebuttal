...
add_log_subdir_argument(parser, 'ggad_2_step_hard_label')
...
with open(output_file, "a") as f:
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            # 转为硬标签
            teacher_hard = to_hard_label(score_from_ggad ,threshold=0.8)
            ...
            if epoch % 5 == 0:
                # Print and save log
                log_message += f"Epoch {epoch}: Total Loss = {total_loss.item()},BCE Loss ={bce_loss.item()}\n"
                model_s.eval()
