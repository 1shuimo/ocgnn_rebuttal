...
add_log_subdir_argument(parser, 'ggad_2_step_score_emb')
...
output_file = get_log_file(args, f"{args.dataset}.txt")
    ...
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            with torch.no_grad():
                ...
                emb_tea_raw_2, _, logits, _, _, emb_tea_raw_1 = model(features, adj, abnormal_label_idx, normal_label_idx, train_flag=True, args=args)
                # 2是最后输出的emb 1 是中间的为了不大的修改模型框架所以加在了最后
                emb_tea_1 = emb_tea_raw_1.squeeze(0)
                emb_tea_1_norm = z_score_normalize(emb_tea_1)
                emb_tea_2 = emb_tea_raw_2.squeeze(0)
                emb_tea_2_norm = z_score_normalize(emb_tea_2)
                

            emb_stu_raw_1, emb_stu_raw_2 = model_s(features, adj)
            
            emb_stu_1 = emb_stu_raw_1.squeeze(0)
            emb_stu_1_norm = z_score_normalize(emb_stu_1)
            emb_stu_2 = emb_stu_raw_2.squeeze(0)
            emb_stu_2_norm = z_score_normalize(emb_stu_2)


            student_score_non_normalize = mlp_s(emb_stu_1).squeeze(dim=-1)
            student_score = torch.sigmoid(student_score_non_normalize)
            score_loss = score_distillation_loss(score_from_ggad, student_score)
            emb_loss = distillation_loss_emb(emb_stu_1_norm, emb_tea_1_norm) + distillation_loss_emb(emb_stu_2_norm, emb_tea_2_norm)

            # student model one class loss
            total_loss = score_loss + emb_loss
            ...
            if epoch % 5 == 0:
                ...
                log_message += f"Score Loss ={score_loss.item()} \n"
                log_message += f"Emb Loss ={emb_loss.item()} \n"
                ...
                emb_score = (mse_loss_1 + mse_loss_2).mean(dim=1)
                ...
                log_message += f'student_emb_score: {torch.mean(emb_score)}\n'
                ...
                log_message += f'Testing {args.dataset} AUC_student_emb: {auc_emb:.4f}\n'
                ...
                log_message += f'Testing AP_student_emb: {AP_emb:.4f}\n'
