    ...
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            with torch.no_grad():
                ...
                emb_t_all, _, logits, _, _ ,_= model(features, adj, abnormal_label_idx, normal_label_idx, train_flag=True, args=args)
            ...
            # 从教师网络的嵌入中提取标记节点的嵌入
            emb_t_normal = emb_t_all[normal_label_idx]

            # 计算标记节点嵌入的均值作为原型
            prototype = torch.mean(emb_t_normal, dim=0)

            # 计算所有节点嵌入与原型的距离
            distances = torch.norm(emb_s_all - prototype, dim=1)

            # 计算距离之和作为正则化项
            regularization_term = torch.sum(distances)

            # 正则化项取均值
            regularization_term = regularization_term / num_nodes
            ...
            total_loss = mean_distillation +  regularization_term
