...
def draw_score_distribution(scores_normal, scores_abnormal, model_type, dataset_name, file_suffix, auroc=None, auprc=None):
    ...
    if auroc is not None and auprc is not None:
        ...
        # 在图例下方添加文本
        ax.text(legend_x, legend_y - 0.5, text_str, fontsize=45, fontweight='bold',
...
output_file = f"./ggad_2_step_2_reg_data_enhance/{args.dataset}_draw_pdf_{args.lr}_only_distill.txt"
...
with torch.no_grad():
    ...
    for mode, (student_scores, teacher_scores) in score_modes.items():
        ...
        if all(len(arr) > 0 for arr in [s_normal, s_abnormal]):
            draw_score_distribution(s_normal, s_abnormal, 'student', args.dataset, f"final_{mode}",auroc=0.951, auprc=0.818)
        else:
            print(f"❌ Skipping student ({mode}) plot - one or more node categories are empty.")

        if all(len(arr) > 0 for arr in [t_normal, t_abnormal]):
            draw_score_distribution(t_normal,t_abnormal, 'teacher', args.dataset, f"final_{mode}",auroc=0.951, auprc=0.818)
        else:
