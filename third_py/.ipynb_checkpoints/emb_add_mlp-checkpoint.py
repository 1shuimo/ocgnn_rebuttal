    ...
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            if epoch % 5 == 0:
                ...
                emb_scores = torch.mean((emb_s - emb_t) ** 2, dim=1)
                mlp_scores = mlp_s(emb_s).squeeze(dim=-1)
                student_scores = emb_scores + mlp_scores
                # Calculate the ocgnn score
                _, score_from_ocgnn = loss_func(emb_s)
                
                total_score = score_from_ocgnn + student_scores
                
                # Extract scores for test indices
                logits = student_scores[idx_test].cpu().detach().numpy()
