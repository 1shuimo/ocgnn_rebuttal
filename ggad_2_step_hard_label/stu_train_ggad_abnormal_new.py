...
output_file = f"./ggad_2_step_new_abnormal/{args.dataset}_abnormal.txt"
with open(output_file, "a") as f:
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            student_score_concat = torch.sigmoid(student_score_concat_non_normalize) # [N+M]


            # 1. 获取ggad硬标签（用未sigmoid的分数）
            teacher_hard = to_hard_label(score_from_ggad_non_normalize, threshold=0.8)  # [N]

            # 2. 找到所有ggad硬标签为1的节点索引
            hard1_indices = torch.where(teacher_hard == 1)[0]  # [K]

            # 输出硬标签为1的节点个数
            num_hard1 = int((teacher_hard == 1).sum().item())
            print(f"ggad硬标签为1的节点个数: {num_hard1}")
            # 3. 取这些节点的emb
            ...
            pseudo_hard = to_hard_label(pseudo_score_from_ggad, threshold=0.8)     # [M]
            ...
            total_loss = bce_loss + 0.1 * reg_loss
