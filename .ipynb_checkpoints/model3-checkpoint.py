...
class Model_ggad(nn.Module):
    def __init__(self, n_in, n_h, activation, negsamp_round, readout):
        ...
        self.mlp_enc = nn.Linear(n_in, n_h, bias=False)
        self.mlp_dec = nn.Linear(n_h, n_h, bias=False)
    ...
    def forward(self, seq1, adj, sample_abnormal_idx, normal_idx, train_flag, sparse=False):
        ...
        # noise = torch.randn(emb_abnormal.size()) * std + mean
        # emb_abnormal = emb_abnormal + noise

        # noise = torch.randn(emb_abnormal.size())
        # noise = 0.001 * noise
        # emb_abnormal = emb_abnormal + noise
        ...
        if train_flag:
            ...
            # TODO CLS
            # emb_combine = torch.cat((emb[:, normal_idx, :], emb[:, sample_abnormal_idx, :]), 1)
            #
            ...
            # TODO VAE Generation
            # emb_con = self.mlp_enc(seq1[0, sample_abnormal_idx, :])
            # emb_con = self.mlp_dec(emb_con)
            # emb_combine = torch.cat((emb[:, normal_idx, :], torch.unsqueeze(emb_con, 0)), 1)
            # #
        ...
        return emb, emb_combine, f_3, emb_con, emb_abnormal, emb_orign
