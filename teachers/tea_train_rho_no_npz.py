...
def main_worker(args):
    ...
    output_file = get_log_file(args, f"{args.dataset}_rho_teacher_no_npz.txt")
    if args.weight_save_path is None:
        weight_dir = ROOT.parent / "rho_teacher_best_pth_no_npz"
        weight_dir.mkdir(parents=True, exist_ok=True)
        args.weight_save_path = str(weight_dir / f"{args.dataset}_rho_teacher_no_npz_best.pth")

    best_auroc = 0.0
    best_summary = ""
    with open(output_file, "a") as f:
        f.write("Using locally constructed Lap from current adjacency.\n")
        f.write("Using split source: official-style split\n")
        with tqdm(total=args.epochs) as pbar:
            pbar.set_description("Training RHO Teacher (no npz)")
    ...
    best_file = get_log_file(args, f"{args.dataset}_rho_teacher_no_npz_best.txt")
...
if __name__ == "__main__":
    ...
    add_log_subdir_argument(parser, "tea_train_rho_no_npz")
