...
with open(output_file, "a") as f:
    with tqdm(total=args.num_epoch) as pbar:
        ...
        for epoch in range(args.num_epoch):
            ...
            AP = average_precision_score(ano_label[idx_test], logits_test, average='macro', pos_label=1, sample_weight=None)

            log_message += f"Epoch {epoch}: Loss = {loss.item():.4f}\n"
            log_message += f'Testing {args.dataset} AUC: {auc:.4f}\n'
            log_message += f'Testing AP: {AP:.4f}\n'

            # ✅ MODIFIED: Save the best model within the specified window, overwriting the previous best
...
if best_epoch_window != -1:
    print(f"✅ Best model in [{target_epoch - window}, {target_epoch + window}] was saved at epoch {best_epoch_window} with AUC {best_auc_window:.4f}.")
    print(f"   Weights saved to: {weight_save_path}")
else:
